...
export function createEngine(pack) {
  ...
  function creditSession(s, now) {
    const a = s.active;
    if (a.sessionRecorded) return;
  ...
  function finish(s, reason = 'complete', now = Date.now()) {
    s.active.stage = 'complete';
    s.active.endReason = reason;
    if (reason === 'complete') creditSession(s, now);
  }
  ...
  function transition(input, command, now = Date.now()) {
    ...
    if (pack.compatibleVersions?.includes(s.packVersion))
      s.packVersion = pack.version;
    ...
    if (
      s.active?.stage === 'complete' &&
      s.active.endReason === 'complete' &&
      !s.active.sessionRecorded
    ) {
      const prior = prog(s, s.active.target);
      prior.sessionsCompleted = Math.max(prior.sessionsCompleted || 0, 1);
      s.active.sessionRecorded = true;
    }
    if (action === 'start') {
    ...
    if (action === 'continue') {
      if (a.stage === 'intro') {
        ...
        if (next === 'retry') {
          ...
          a.repairs++;
          if (a.repairs > 3) finish(s, 'pause', now);
          ...
          if (a.done >= a.goal && !a.returnTransfer) finish(s, 'complete', now);
          else targetQuestion(s, now);
        }
    ...
    const wasCompleted = !!prog(s, q.skill).completed;
    ...
    if (correct) {
      ...
      if (a.role === 'diagnostic') {
        ...
        if (a.done >= a.goal && !a.returnTransfer) creditSession(s, now);
        next = 'next';
      }
    } else {
      a.failures++;
    ...
    a.feedback = {
      ...
      milestoneCompleted: !wasCompleted && !!prog(s, q.skill).completed,
      answer: command.answer,
    };
    return s;
  }
  function progressView(s, id) {
    const p = { ...blankProgress(), ...s.progress[id] };
    // Old packs did not store per-milestone session totals. Only recover a session
    // if the active saved completion explicitly proves it; never infer mastery.
    if (
      !p.sessionsCompleted &&
      s.active?.target === id &&
      s.active.stage === 'complete' &&
    ...
    const results = p.window
      .filter((x) => x.band === p.band)
      .map((x) => x.correct)
      .slice(-3);
    p.checkpoint = {
      attempted: results.length,
      correct: results.filter(Boolean).length,
      results: [...results, ...Array(3 - results.length).fill(null)],
    };
    p.solved = Object.entries(s.history).filter(
      ([qid, h]) => h.rewarded && questions[qid]?.skill === id,
    ).length;
    p.status = p.completed
      ? 'completed'
      : p.sessionsCompleted
        ? 'practised'
        : p.introduced || p.attempts
          ? 'in-progress'
          : 'new';
    return p;
  }
  function view(s, now = Date.now()) {
      ...
      active = {
        ...
        question: q
          ? {
              ...
              visual:
                a.hints > 0 || a.failures > 0 || a.stage === 'feedback'
                  ? q.visual
                  : undefined,
              source: q.source,
    ...
    return {
      ...
      progress: Object.fromEntries(
        pack.skills.map((x) => [x.id, progressView(s, x.id)]),
      ),
      active,
